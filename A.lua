...
local Dropdown = PartLabel:AddDropdown({
    ...
    Callback = function(Option)
        if Option == "Head" then
            ArchondAimbotSettings.TargetPart = "Head"

        elseif Option == "HumanoidRootPart" then 
            ArchondAimbotSettings.TargetPart = "HumanoidRootPart"

        elseif Option == "Random" then
            local valid = {"Head", "HumanoidRootPart", "Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg"}
            local randomIndex = math.random(1, #valid)
            ArchondAimbotSettings.TargetPart = valid[randomIndex]
        end 
    end
}, "RandomPart")
